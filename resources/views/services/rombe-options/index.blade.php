...
@section('title', 'Manajemen Opsi Rombe')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0"><i class="fas fa-gem me-2"></i>Manajemen Opsi Rombe</h2>
    <a href="{{ route('services.rombe-options.create') }}" class="btn btn-primary">
...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body">
        <form action="{{ route('services.rombe-options.index') }}" method="GET" class="row g-3">
            <div class="col-md-6">
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Daftar Opsi Rombe</h5>
        <div>
            <span class="text-muted">Menampilkan {{ $rombeOptions->firstItem() ?? 0 }} sampai {{ $rombeOptions->lastItem() ?? 0 }} dari {{ $rombeOptions->total() }} data</span>
        </div>
    </div>
    <div class="card-body p-0">
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    @forelse($rombeOptions as $rombeOption)
                    <tr>
                        <td>{{ $rombeOptions->firstItem() + $loop->index }}</td>
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="rombe-icon bg-primary text-white me-2">
                        ...
                        <td>{{ $rombeOption->created_at->format('d M Y') }}</td>
                        <td>
                            <div class="btn-group" role="group">
                                <a href="{{ route('services.rombe-options.show', $rombeOption) }}" 
                                   class="btn btn-sm btn-outline-info" 
                                   title="Lihat">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ route('services.rombe-options.edit', $rombeOption) }}" 
                                ...
                                <button type="button" 
                                        ...
                                        onclick="showDeleteModal({{ $rombeOption->id }}, '{{ $rombeOption->color }} - {{ $rombeOption->size_indonesia }}')"
                                        title="Hapus">
                                    <i class="fas fa-trash"></i>
                                </button>
                                <form id="delete-form-{{ $rombeOption->id }}" 
                                      action="{{ route('services.rombe-options.destroy', $rombeOption) }}" 
                                      method="POST" 
                                      class="d-none">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-4">
                            ...
                            <p class="text-muted mb-0">Tidak ada opsi rombe ditemukan</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    @if($rombeOptions->hasPages())
    <div class="card-footer bg-white">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                Menampilkan {{ $rombeOptions->firstItem() }} sampai {{ $rombeOptions->lastItem() }} dari {{ $rombeOptions->total() }} data
            </div>
            <div>
                {{ $rombeOptions->links() }}
            </div>
...
@push('scripts')
<script>
let deleteRombeOptionId = null;

function showDeleteModal(rombeOptionId, rombeOptionName) {
    deleteRombeOptionId = rombeOptionId;
...
document.getElementById('confirmDeleteBtn')?.addEventListener('click', function() {
    if (deleteRombeOptionId) {
        this.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i>Menghapus...';
        this.disabled = true;
        document.getElementById('delete-form-' + deleteRombeOptionId).submit();
    }
});

document.getElementById('deleteModal')?.addEventListener('hidden.bs.modal', function() {
    deleteRombeOptionId = null;
