...
@section('title', 'Detail Opsi Pita Motif')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="mb-0"><i class="fas fa-wind me-2"></i>Detail Opsi Pita Motif</h2>
    <div>
        <a href="{{ route('services.motif-ribbon-options.edit', $motifRibbonOption) }}" class="btn btn-primary me-2">
...
<div class="row">
    <!-- Motif Ribbon Option Info Card -->
    <div class="col-md-4">
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body text-center">
                <div class="motif-ribbon-icon-large bg-primary text-white mx-auto mb-3">
                    <i class="fas fa-wind fa-3x"></i>
                </div>
                <h4 class="mb-1">{{ $motifRibbonOption->color }}</h4>
                <p class="text-muted mb-2">Ukuran: {{ $motifRibbonOption->size_indonesia }}</p>
                <h3 class="text-success mb-3">{{ $motifRibbonOption->formatted_price }}</h3>
                ...
                <div class="d-grid gap-2">
                    <a href="{{ route('services.motif-ribbon-options.edit', $motifRibbonOption) }}" class="btn btn-primary">
                        <i class="fas fa-edit me-2"></i>Edit Opsi
                    </a>
                    
                    @if(!$motifRibbonOption->isUsedInOrders())
                    <button type="button" class="btn btn-danger" onclick="showDeleteModal('{{ $motifRibbonOption->color }} - {{ $motifRibbonOption->size_indonesia }}')">
                        <i class="fas fa-trash me-2"></i>Hapus Opsi
                    </button>
                    <form id="delete-form" action="{{ route('services.motif-ribbon-options.destroy', $motifRibbonOption) }}" method="POST" class="d-none">
                        @csrf
                        @method('DELETE')
                    </form>
                    @else
    ...
    <!-- Motif Ribbon Option Details -->
    <div class="col-md-8">
        <!-- Motif Ribbon Option Information -->
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white">
                <h6 class="mb-0"><i class="fas fa-info-circle me-2"></i>Informasi Opsi Pita Motif</h6>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <small class="text-muted d-block">ID Opsi</small>
                        <strong>#{{ $motifRibbonOption->id }}</strong>
                    </div>
                    <div class="col-md-6 mb-3">
                        <small class="text-muted d-block">Warna/Motif</small>
...
.motif-ribbon-preview-container {
    background: #f8f9fa;
    border-radius: 8px;
    padding: 2rem;
}
...
.motif-ribbon-sample[data-size="small"] .ribbon-wave {
    height: 20px;
}

.motif-ribbon-sample[data-size="medium"] .ribbon-wave {
    height: 35px;
}

.motif-ribbon-sample[data-size="large"] .ribbon-wave {
    height: 50px;
}
...
@push('scripts')
<script>
function showDeleteModal(motifRibbonOptionName) {
